...
if(isset($_POST['Update'])){
	if(mysqli_query($conn,"UPDATE `cal_events_new` SET `event_name`=$name, `event_desc`=$description, `web_user`=$logged_in_user, `category`=$category WHERE `location`=$location")){
			echo '<br>updated';
		}
		else{
			echo mysqli_error($conn);
